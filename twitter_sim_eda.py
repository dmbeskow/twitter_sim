...
F = create_polarized_network()
draw_simulation(F)
for node, data in F.nodes(data=True):
    print(node,':',data['belief'])
...
#                data['belief'] = data['belief'] + data['susceptibility'] * perc * (1-data['belief'])
            ...
            if data['kind'] == 'bot':
                chance = 0.8
                tweets = list(choice(bucket1, np.random.randint(2,10),p=[1-chance, chance]))
...
#                chance = data['belief'] * influence_proportion
                ...
                tweets = list(choice(bucket1, np.random.randint(0,5),p=[1-chance, chance]))
...
#for file in files:
#    os.rename(file, re.sub('target','back',file))